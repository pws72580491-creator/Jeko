...
async function confirmEdit() {
  ...
  const prevDate = t.date; // 날짜 변경 전 원본 저장
  ...
  if(t.category==='make') {
    const src=FINISHED_SOURCE[item];
    const autoKey='[자동] ';
    // prevDate로 검색해야 날짜 변경 시에도 정확히 찾음
    const conIdx=(state.transactions||[]).findIndex(x=>x&&x.category==='consumption'&&x.date===prevDate&&x.memo&&x.memo.startsWith(autoKey));
    if(src) {
      const need=Math.ceil(pcs*src.ratio);
      const u=pcsToUnits(need,src.source);
      if(conIdx>=0) {
